...
void main()
{
  ...
  const float r_inner = Params.innerFOV / Params.outerFOV;

  const bool visible = posLightSpaceNDC.z < textureLod(shadowMap, shadowTexCoord, 0).x + 0.001f;
  
  float shadow = 0.0f;
  const float dist = length(posLightSpaceNDC.xy);
  if (dist < r_inner)
    shadow = 1.0f;
  else if (dist > r_outer)
    shadow = 0.0f;
  else // between r_inner and r_outer
    shadow = (r_outer - dist) / (r_outer - r_inner);
  
  shadow = visible ? shadow : 0.0f;
